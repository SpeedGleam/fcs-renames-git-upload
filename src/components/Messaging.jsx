...
export default function Messaging() {
    ...
    const fetchData = async () => {
      try {
        ...
        const sortedUsers = [...usersResponse.data].sort((a, b) => {
          if (a.last_interaction === null && b.last_interaction === null)
            return 0;
  ...
  const isCurrentUserSender = (message) =>
    message.sender === user?.id || message.sender_username === user?.username;
      ...
      <div className="chat-main">
          ...
          {filteredMessages.length > 0 ? (
            filteredMessages.map((msg) => (
              <div
                key={msg.id}
                className={`message ${
                  isCurrentUserSender(msg) ? "sent" : "received"
                }`}
              >
                <div className="message-content">
                  ...
                  {(!msg.attachment_url ||
                    (msg.content &&
                      msg.content.toLowerCase() !== "attachment")) && (
        ...
        <form className="message-input" onSubmit={sendMessage}>
          ...
          <input
            ...
            id="attachmentInput"
          />
          <label htmlFor="attachmentInput" className="attach-button">
            <i className="fas fa-paperclip"></i>
            {attachment && <span className="attachment-indicator"></span>}
          </label>
          <button
            type="submit"
            className="send-button"
            disabled={loading || !receiverId || (!newMessage.trim() && !attachment)}
          >
