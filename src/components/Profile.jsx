  ...
  const handleUpdateProfile = async (e) => {
    ...
    if (profileData.profile_picture) {
      ...
      if (!allowedPicTypes.includes(profileData.profile_picture.type)) {
        return setError("Only JPEG, PNG, and GIF allowed for profile picture");
      }
    ...
    if (profileData.verification_document) {
      ...
      if (!allowedDocTypes.includes(profileData.verification_document.type)) {
        return setError("Only PDF, JPG, and PNG files allowed for verification document");
      }
      if (profileData.verification_document.size > 5 * 1024 * 1024) {
        return setError("Verification document must be less than 5MB");
      }
      formData.append('verification_document', profileData.verification_document);
    }
    ...
    try {
      ...
      const message = error.response?.data?.detail ||
        error.request ? "No server response" :
      ...
      <form onSubmit={handleUpdateProfile} className="auth-form">
        ...
        <div className="form-group verification-status">
          ...
          <div className="status-indicator">
            <i className={`fas fa-${profileData.is_verified ? 'check-circle verified' : 'times-circle unverified'}`}></i>
            <span style={{ color: profileData.is_verified ? 'green' : 'red' }}>
